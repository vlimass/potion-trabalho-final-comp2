...
import static com.example.trabalhofinal.PotionApplication.blocos;

@Path("/editarposicao")
public class EditarPosicaoResource {
    @GET
    @Produces("text/html")
    public String editarposicao(@QueryParam("posicao_inicial") @DefaultValue("") String posicao_inicial, @QueryParam("posicao_final") @DefaultValue("") String posicao_final) {
        int posicaoI = Integer.parseInt(posicao_inicial);
        int posicaoF = Integer.parseInt(posicao_final);
        posicaoF--;

        try{
            if(posicaoI == posicaoF){
                throw new Exception("O bloco já está nessa posição");
            }

            if(posicaoI < posicaoF){
                while(posicaoI < posicaoF){
                    Collections.swap(blocos, posicaoI, posicaoI + 1);
                    posicaoI++;
                }
            }

            if(posicaoI > posicaoF){
                while(posicaoF < posicaoI){
                    Collections.swap(blocos, posicaoI, posicaoI - 1);
                    posicaoI--;
                }
            }
            ...
            String html="<!DOCTYPE html>\n" +
                    ...
                    "        <p class=\"first-line\">Posição alterada com sucesso!</p>\n" +
            ...
            String html = "<!DOCTYPE html>\n" +
                    ...
                    "        <meta http-equiv= \"X-UA-Compatible\" content = \"IE=edge\">\n" +
                    "        <meta name = \"viewport\" content = \"width=device-width, initial-scale=1.0\">\n" +
                    "        <title>Erro! Anotação na mesma posição</title>\n" +
                    //FAVICON:
                    "   <link rel=\"shortcut icon\" href=\"https://media.discordapp.net/attachments/890312701123895377/949731505540845638/icon_2.png\" type=\"image/x-icon\">"+
                    //CSS:
                    "$cssStyle\n" +
                    ...
                    "           <p class=\"first-line\">Essa anotação já está nessa posição!</p>\n" +
                    "           <span class=\"buttons\">\n" +
                    "               <a href = \"posicao?posicao_inicial="+posicao_inicial+"\" class=\"first-link\">Voltar</a>" +
